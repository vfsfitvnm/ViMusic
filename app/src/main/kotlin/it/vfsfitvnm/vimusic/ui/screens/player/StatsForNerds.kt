        ...
        Box(
            ...
            Row(
                ...
                Column {
                    ...
                    BasicText(
                        text = format?.bitrate?.let { "${it / 1000} kbps" } ?: stringResource(id = R.string.unknown),
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                    BasicText(
                        text = format?.contentLength
                            ?.let { Formatter.formatShortFileSize(context, it) } ?: stringResource(id = R.string.unknown),
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                    BasicText(
                    ...
                    BasicText(
                        text = format?.loudnessDb?.let { "%.2f dB".format(it) } ?: stringResource(id = R.string.unknown),
                        maxLines = 1,
                        style = typography.xs.medium.color(colorPalette.onOverlay)
                    )
                }
